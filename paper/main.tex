...
ablating a random set of the same size. That holds on GPT-2 Small, and the
induction result reproduces on GPT-2 Medium and Pythia-410M. The extract step
costs one forward; a head-by-head patch sweep costs about two orders of
magnitude more.
...
\]
Raw attention alone is a backup score; the product works a bit better in our
checks.
...
Pythia-410M~\cite{biderman2023pythia}. Induction prompts look like
\texttt{The cat sat. The cat} $\to$ \texttt{sat}. IOI uses the usual name
templates on Small. Same recipe everywhere: product score, \(k{=}15\), four
hops, ten random controls.

\subsection{The sketch beats random}

Figure~\ref{fig:causal} and Table~\ref{tab:main} are the headline. On GPT-2
Small induction, turning off \(S\) drops the target logit by about \(3.7\) on
average; random edges of the same size barely move it (\(\sim 0.1\)). Every one
of \(23\) prompts passes. Medium and Pythia show the same pattern with smaller
gaps. IOI on Small is even cleaner: gap about \(5.2\), again \(0/23\) fails.
So this is not a one-task trick.
...
\begin{table}[t]
  \centering
  \caption{Same story in numbers. Gap $=\Delta_S-\Delta_R$; higher is better
  for our claim.}
  \label{tab:main}
...
Out of \(92\) prompt-level runs across tasks and models, five fail the random
test (\(\sim\)5\%), all on Medium or Pythia induction. Small stays clean.
Typical failure modes: the walk latches onto a high-scoring distractor, the
fixed hop budget misses a longer path, or attention mass is not causation
(hence the BOS ban and the value-norm score).
...
is necessary or monosemantic~\cite{marks2025sfc,ameisen2025circuittracing}. Heavier methods
remain the right tool when you need completeness
\cite{conmy2023acdc,ferrando2024ifr}. Our wedge is simpler: attention already draws a
map; a short walk on that map is often enough to find something that matters,
fast.
